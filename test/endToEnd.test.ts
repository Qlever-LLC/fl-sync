  ...
  */
  ...
  docsWatch.on(ChangeType.ItemChanged, async ({ change }) => {
    ...
    if (ch.body?.['food-logiq-mirror']?.shareSource?.originalId) {
      // @ts-expect-error
      supplierIdToCommunityId.set(ch.body['food-logiq-mirror'].shareSource.originalId, ch.body['food-logiq-mirror']._id);
    }
  });
...
test(`End to end basic COI approval`, async (t) => {
  t.timeout(150_000);
  const testDoc = Object.assign({}, doc['food-logiq-mirror']) as {
    [k: string]: any;
    _id?: string;
  };
  delete testDoc._id;
...
test(`End to end multiple COIs in one PDF should be left alone`, async (t) => {
  ...
  delete testDoc._id;
  const docResponse = await postDocument(baseDoc, 'COI-MultipleCOIs_OnePDF.pdf');
...
test.skip(`Should correctly handle a doc that gets approved "accidentally" then rejected in quick succession by a SF user`, async (t) => {
  ...
  t.truthy(awaitingApprovalJob?.value?.result?.message?.includes('interrupted'));
...
async function postDocument(document: any, filename: string) {
  const file = fs.readFileSync(`./test/pdfs/${filename}`, { encoding: 'utf-8' });
...
const gfsicert = {
  ...
  "auditAttributes": {
    ...
    "reAuditDate": null,
    "certificationBody": "",
    "scope": "GlobalG.A.P - Integrated Farm Assurance Standard (GFSI) - GlobalG.A.P - Integrated Farm Assurance Standard (GFSI)",
