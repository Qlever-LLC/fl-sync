...
// @ts-expect-error csvjson lacks types
...
async function processCoi(coi: any, pdfs: any) {
  ...
  for await (const [key, { _id }] of Object.entries(attachments)) {
...
*/
function combineCois(cois: TrellisCOI[]): TrellisCOI {
...
async function writeExcelFile(rows: Array<Record<string, ExcelRow>>) {
  ...
  */
  ...
  worksheet.getColumn(4).font = { bold: true, name: 'Calibri' };
 ...
 *
 */
async function recombineGenerateCoisReport(path: string) {
  const json = readFileSync(path, 'utf8');
  const data = JSON.parse(json);
  const results : Array<Record<string, ExcelRow>> = [];

  let grouped : Record<string, any[]> = groupBy(
    data as any[],
    (item) => item.coi?.shareSource?.sourceBusiness?.name
  )
...
function groupBy(items: any[], func: (key:any)=> string) {
...
recombineGenerateCoisReport(filename);