...
async function fakeFlBusiness(con: OADAClient) {
  ...
  const mid = `TRELLIS-TEST${ksuid.randomSync().string}`;
  await con.put({
...
function testNonCoiDocument(bid: string) {
  return {
    products: [],
    locations: [],
    ...
    shareRecipients: [
      {
        ...
        type: {
          ...
          helpText:
            'Please upload your current 3rd Party Animal Welfare Audit.',
          associateWith: '',
...
function testCoiDocument(bid: string) {
  return {
    products: [],
    locations: [],
    contentType: 'document',
...
export default {
  fakeFlBusiness,
  testNonCoiDoc: testNonCoiDocument,
  testCoiDoc: testCoiDocument,
  newCoiDoc: newCoiDocument,
};
