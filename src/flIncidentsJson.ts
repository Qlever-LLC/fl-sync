...
import type { FlPage } from './index.js';
import tree from './tree.js';
import { FlObject } from './mirrorWatch.js';
...
export async function fetchIncidentTypes({
  ...
  const data = await got({
    ...
    headers: { Authorization: FL_TOKEN },
    searchParams: pageIndex ? { pageIndex } : undefined,
  }).json<{ has_more: boolean; total: number; items: FlIncidentType[] }>();
...
export interface FlStatusHistoryItem {
...
export interface FlIncidentType {
  createdAt: string;
  updatedAT: string;
  business: FlBusiness;
  _id: string;
  name: string;
  incidentsConf: FlIncidentsConfig[];
...
export interface FlIncidentsConfig {
  id: number;
  name: string;
  condition: string;
  conditionVal: string;
