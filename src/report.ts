...
const humanReadableJobError = {
  ...
  'target-validation': 'Required data elements missing during Target extraction',
  'target-multiple-docs-combined': 'The PDF contained multiple document extractable results, but this is current not supported.',
  'bad-fl-attachments':
...
export async function makeFinalReport() {
  ...
  try {
    ...
    await Promise.each(busKeys, async (bid: string) => {
      ...
      const docKeys = Object.keys(docs).filter(index => !index.startsWith("_"));

      await Promise.each(docKeys, async (docid: string) => {
        if (!docid) return;
        ...
        let busName = _.get(doc, "food-logiq-mirror.shareSource.sourceBusiness.name");
        let status = _.get(doc, "food-logiq-mirror.shareSource.approvalInfo.status");
        let user = _.get(doc, "food-logiq-mirror.shareSource.approvalInfo.setBy._id");
        ...
        if (status) {
          ...
          finalReport.flStatuses[status].total = finalReport.flStatuses[status].total || 0;
          finalReport.flStatuses[status].total++;
        }
        if (status && user && user === FL_TRELLIS_USER) {
          finalReport.flStatuses[status].byUs = finalReport.flStatuses[status].byUs || 0;
        ...
        if (Object.keys(jobs).length <= 0) {
          ...
          finalReport.flStatuses[status].missingFlSyncJob = finalReport.flStatuses[status].missingFlSyncJob || 0;
          finalReport.flStatuses[status].missingFlSyncJob++;
          otherReport.missingFlSyncJobs = otherReport.missingFlSyncJobs || {};
          otherReport.missingFlSyncJobs[status] = otherReport.missingFlSyncJobs[status] || {};
          otherReport.missingFlSyncJobs[status][docid] = {docid, bid, status, user};
          if (status === 'approved') {
            console.log('~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~')
            console.log('~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~')
            console.log('~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~')
        ...
        if (jobError) {
          finalReport["job-errors"][jobError] = finalReport["job-errors"][jobError] || 0;
        ...
        if (coiId) {
          ...
          if (entryId) {
            ...
            finalReport.flStatuses[status].notInLaserfiche = finalReport.flStatuses[status].notInLaserfiche || 0;
            finalReport.flStatuses[status].notInLaserfiche++;
            otherReport.notInLaserfiche[docid] = {docid, bid, jobid};
            if (status === 'approved') {
              otherReport.approvedNotInLaserfiche[docid] = {docid, bid, jobid}
            }
          }
        }
    ...
    fs.writeFileSync('./scripts/finalReportDocs-Prod.json', JSON.stringify(otherReport));
    fs.writeFileSync('./scripts/finalReport-Prod.json', JSON.stringify(finalReport));
    finalReport.checks = {
      totals: {
        ...
        sum: finalReport.ignoreTestSupplier+finalReport.jobSuccess+ finalReport["job-errors"].total + finalReport.otherErrors + finalReport.missingFlSyncJob,
      },
      awaiting: {
        ...
        'missingFlSyncJob': finalReport.flStatuses['awaiting-review'].missingFlSyncJob,
        'bad-fl-attachments': finalReport['job-errors']['bad-fl-attachments'],
        'otherErrors': finalReport.otherErrors,
        sum: finalReport['job-errors']['target-validation']+finalReport['job-errors']['target-unrecognized']+finalReport['job-errors']['target-other']+finalReport.flStatuses['awaiting-review'].missingFlSyncJob+finalReport['job-errors']['bad-fl-attachments']+finalReport.otherErrors,
      },
      approved: {
...
export async function isItTime(oada: OADAClient) {
  ...
  const now = moment();
  const reportTime = await oada.get({
  ...
  if (
    lastTime.year() === now.year() && lastTime.clone().add(1, 'day').dayOfYear() === now.dayOfYear()
  ) {
